...
if [ "PROTRACTOR_HOST" == "" ]; then
    echo "env variable PROTRACTOR_HOST is not defined!"
    exit -1
fi
sudo -u protractor xvfb-run --server-args="-screen 0 ${SCREEN_RES}" -a protractor --baseUrl="${PROTRACTOR_HOST}" $@
